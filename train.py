...
def get_ds(config):
    ...
    train_ds_size = int(0.90 * len(ds_raw))
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        model.train()
        batch_itrator = tqdm.tqdm(train_loader, desc=f'Processing epoch {epoch:02d}')
        ...
        for batch in batch_itrator:
            ...
            batch_itrator.set_postfix({f'loss': f'{loss.item():6.3f}'})
